...
if __name__ == "__main__":
    ...
    file1Input = input("Enter Revision: ")
    while path.isfile(file1Input) == False:
        file1Input = input("Please Re-enter Revision:")
    else:
        file2Input = input("Enter OMRON Text File: ") 
        while path.isfile(file2Input) == False:
            file2Input = input("Please OMRON Text File:")
    ...
    with open(file3Output, 'w') as newFile:
        ...
        # VERIFY VERSION NUMBER FOR EACH UPDATE
        newFile.write("Omron Verification Tool Version: 1.0")
        now = datetime.now()
        ...
        # If not the same, print the removed, added, and changed dicitonaries
            ...
            if len(removedInNextRevision) > 0:
                newFile.write("\n\To Add " + repr(len(removedInNextRevision)) + " Items in OMRON File")
                tempString = printDict(removedInNextRevision)
                newFile.write(tempString)

            if len(addedInNextRevision) > 0:
                newFile.write("\nTo Remove " + repr(len(addedInNextRevision)) + " Items in OMRON File")
                tempString = printDict(addedInNextRevision)
                newFile.write(tempString)

            if len(changedInNextRevision) > 0:
                newFile.write("\nTo Change " + repr(len(changedInNextRevision)) + " Items in OMRON File")
