...
if __name__ == "__main__":
    printBanner()
    print("Files MUST be within the same folder and include '.prg'")
    ...
    changedInNextRevision = refAndIpn.changedInNext(refAndIpnDict1, refAndIpnDict2)
    ...
    with open(file3Output, 'w') as newFile:
        ...
        newFile.write("Date Created: " + repr(dt_string))

        if refAndIpn.checkSame(refAndIpnDict1, refAndIpnDict2):
            newFile.write("\n")
            newFile.write("NO CHANGES IN NEXT REVISION")
        else:
            #print("Unchanged In Next Revision:")
            #printDict(unchangedInNextRevision)

            newFile.write("\n")
            newFile.write("Rev1: " + repr(file1Input) + "\tLines: " + repr(len(refAndIpnDict1)))
            newFile.write("\nRev1: " + repr(file2Input) + "\tLines: " + repr(len(refAndIpnDict2)))

            newFile.write("\n\nCHANGES:")
            if len(removedInNextRevision) > 0:
                newFile.write("\n\nRemoved " + repr(len(removedInNextRevision)) + " Items in Next Revision (Removed from "+ repr(file1Input) + "): ")
                tempString = printDict(removedInNextRevision)
                newFile.write(tempString)

            if len(addedInNextRevision) > 0:
                newFile.write("\nAdded " + repr(len(removedInNextRevision)) + " Items in Next Revision (Added to "+ repr(file2Input) + "): ")
                tempString = printDict(removedInNextRevision)
                newFile.write(tempString)

            if len(unchangedInNextRevision) > 0:
                newFile.write("\nChanged " + repr(len(changedInNextRevision)) + " Items in Next Revision (Changed from " + repr(file1Input) + " to " + repr(file2Input) + "): ")
                tempString = refAndIpn.printChangedInNext(changedInNextRevision)
                newFile.write(tempString)

            newFile.close()
            # this keeps the .exe open after user inputs, and lets user know if new file has been created.
            exitRequest = ""
            if path.isfile(file3Output):
                print("A new file: '" + file3Output + "' has succesfully been created.")
                exitRequest = input("")
            else:
                print("No new file has been created. Format of input files are not compatible or have changed.")
                exitRequest = input("")
