...
if __name__ == "__main__":
    ...
    unchangeddInNextRevision = refAndIpn.unchangedInNext(refAndIpnDict1, refAndIpnDict2)
    ...
    with open(file3Output, 'w') as newFile:
        ...
        # VERIFY VERSION NUMBER FOR EACH UPDATE
        newFile.write("Revision Compare Tool Version: 1.2")
        now = datetime.now()
        dt_string = now.strftime("%b-%d-%Y %H:%M:%S")
        newFile.write("\nDate Created: " + repr(dt_string))
        newFile.write("\n\nRev1: " + repr(file1Input) + "\tLines: " + repr(len(refAndIpnDict1)))
        newFile.write("\nRev2: " + repr(file2Input) + "\tLines: " + repr(len(refAndIpnDict2)))

        # Checking if the two revisions are the same. 
        if refAndIpn.checkSame(refAndIpnDict1, refAndIpnDict2):
            newFile.write("\n\nNo Changes Between " + repr(file1Input) + " and " + repr(file2Input))
        ...
        # If not the same, print the removed, added, and changed dicitonaries
            ...
            if len(addedInNextRevision) > 0:
                newFile.write("\nAdded " + repr(len(addedInNextRevision)) + " Items in Next Revision (Added to "+ repr(file2Input) + "): ")
                tempString = printDict(addedInNextRevision)
                newFile.write(tempString)

            if len(unchangeddInNextRevision) > 0:
        ...
        if path.isfile(file3Output):
            print("A new file: '" + file3Output + "' has succesfully been created. Program will close in 5 seconds.")
        else:
